...
obstruction_shape = 'sphere' #box, cylinder, sphere
...
def RandomObstructions(num=range(10), width=600, length=600, height=600, draw=True):
	
	#floor = box(pos=(0, 0, 0), length=width, width=length, height=1)
	#ceiling = box(pos=(0, height, 0), length=width, width=length, height=1)
	obstruction = {}
		
	for obs in num:
		tempWidth = random.randint(1, width/2)
		tempHeight = random.randint(1, height/2)
		tempLength = random.randint(1,10)
		
##		tempX = random.randint(0, width) - width/2
##		tempY = random.randint(0, height)
##		tempZ = random.randint(0, length) - length/2
		tempX = random.randint(0, width)
		tempY = random.randint(0, height)
		tempZ = random.randint(0, length)
		
		#print "obs: {3} x: {0} y: {1}, z: {2} w: {4} h: {5} l:{6}".format(tempX, tempY, tempZ, obs, tempWidth, tempHeight, tempLength)

		#commented - box obstruction, added cylindrical obstrcution
		if obstruction_shape == 'box':
			obstruction[obs] = box(pos=(tempX,tempY,tempZ), length=tempWidth, width=tempLength, height=tempHeight, color=color.red, visible=(True if draw else False))
		elif obstruction_shape == 'cylinder':
			obstruction[obs] = cylinder(pos=(tempX,tempY,tempZ), radius=tempWidth/8, length=tempLength*8, color=color.red, visible=(True if draw else False))
		else:
			obstruction[obs] = sphere(pos=(tempX,tempY,tempZ), radius=tempWidth/4, color=color.red, visible=(True if draw else False))
	return obstruction


def RandomGraphWithObstructions(obs, width=600, length=600, height=600, density=100, drawConnections=True, draw=True):
	...
	g.reverseLookup = {}
	count = 0
##	rWidth = range(-width/2 - 1, width/2, density)
##	rLength = range(-length/2 - 1, length/2, density)
##	rHeight = range(0, height + 1, density)
	rWidth = range(0, width + 1, density)
	rLength = range(0, length + 1, density)
	rHeight = range(0, height + 1, density)
	cost = 1

	g.bbox3 = [0, width, 0, height, 0, length]
	g.scale = density
	g.obstructions = obs
	g.size = [width,height,length]

	# First, generate all of the nodes as long as they are not in an obstruction
	for l in rLength:
		for w in rWidth:
			for h in rHeight:
				if (not nodeInObstruction(obs, w, h, l)):
					g.locations[count] = (w, h, l)
					g.objects["x:{0}y:{1}z:{2}".format(w, h, l)] = sphere(pos=(w, h, l), radius = 2, color=color.blue, visible=(False if draw else False))
					g.lookup["x:{0}y:{1}z:{2}".format(w, h, l)] = count
					g.reverseLookup[count] = "x:{0}y:{1}z:{2}".format(w, h, l)
					count = count + 1
	
	# Now, connect adjacent nodes as long as there is no obstacle in between them
	for key, value in g.objects.items():

		# Check to see if there is a node in front of the current node
		cKey = "x:{0}y:{1}z:{2}".format(int(value.pos.x), int(value.pos.y), int(value.pos.z + density))
		if cKey in g.objects:
			if not newPathInObstruction(true, obs, value.pos.x, value.pos.y, value.pos.z, 0, 0, density):
				g.connect(g.lookup[key], g.lookup[cKey], cost)
				if drawConnections:
					arrow(pos=(value.pos.x, value.pos.y, value.pos.z), axis=(0, 0, density), shaftwidth=0.5, fixedwidth=true)
			
		# Check to see if there is a node above the current node
		cKey = "x:{0}y:{1}z:{2}".format(int(value.pos.x), int(value.pos.y + density), int(value.pos.z))
		if cKey in g.objects:
			g.connect(g.lookup[key], g.lookup[cKey], cost)
			if drawConnections:
				arrow(pos=(value.pos.x, value.pos.y, value.pos.z), axis=(0, density, 0), shaftwidth=0.5, fixedwidth=true)
			
		# Check to see if there is a node to the right of the current node
		cKey = "x:{0}y:{1}z:{2}".format(int(value.pos.x + density), int(value.pos.y), int(value.pos.z))
		if cKey in g.objects:
			g.connect(g.lookup[key], g.lookup[cKey], cost)
			if drawConnections:
				arrow(pos=(value.pos.x, value.pos.y, value.pos.z), axis=(density, 0, 0), shaftwidth=0.5, fixedwidth=true)
	
		# Check to see if there is a node to the front-right of the current node
		cKey = "x:{0}y:{1}z:{2}".format(int(value.pos.x + density), int(value.pos.y + density), int(value.pos.z - density))
		if cKey in g.objects:
			if not newPathInObstruction(false, obs, value.pos.x, value.pos.y, value.pos.z, density, density, -density):
				g.connect(g.lookup[key], g.lookup[cKey], cost)
				if drawConnections:
					arrow(pos=(value.pos.x, value.pos.y, value.pos.z), axis=(density, density, -density), shaftwidth=0.5, fixedwidth=true)

		# Check to see if there is a node to the front-left of the current node
		cKey = "x:{0}y:{1}z:{2}".format(int(value.pos.x - density), int(value.pos.y + density), int(value.pos.z - density))
		if cKey in g.objects:
			if not newPathInObstruction(false, obs, value.pos.x, value.pos.y, value.pos.z, -density, density, -density):
				g.connect(g.lookup[key], g.lookup[cKey], cost)
		...
		# Check to see if there is a node to the bottom-left of the current node
		cKey = "x:{0}y:{1}z:{2}".format(int(value.pos.x - density), int(value.pos.y - density), int(value.pos.z - density))
		if cKey in g.objects:
			if not newPathInObstruction(false, obs, value.pos.x, value.pos.y, value.pos.z, -density, -density, -density):
				g.connect(g.lookup[key], g.lookup[cKey], cost)
		...
		# Check to see if there is a node to the bottom-right of the current node
		cKey = "x:{0}y:{1}z:{2}".format(int(value.pos.x + density), int(value.pos.y - density), int(value.pos.z - density))
		if cKey in g.objects:
			if not newPathInObstruction(false, obs, value.pos.x, value.pos.y, value.pos.z, density, -density, -density):
				g.connect(g.lookup[key], g.lookup[cKey], cost)
...
def newPathInObstruction(includeZ, obstructions, x1, y1, z1, x2, y2, z2):

	for key, value in obstructions.items():
		if obstruction_shape == 'box':
			if (z1 <= (value.pos.z + value.width/2) and z1 + z2 >= (value.pos.z + value.width/2)) or not includeZ:

				v = [
					# Top left
					[value.pos.x - value.length/2, value.pos.y + value.height/2, value.pos.z - value.width/2], 
					# Bottom left
					[value.pos.x - value.length/2, value.pos.y - value.height/2, value.pos.z - value.width/2], 
					# Bottom right
					[value.pos.x + value.length/2, value.pos.y - value.height/2, value.pos.z - value.width/2], 
				...
				ray = Ray(position=(x1, y1, z1), direction=(x2, y2, z2))
				if not polygon.intersection(ray) is None:
					return true
				
		elif obstruction_shape == 'cylinder':
			if (z1 <= (value.pos.z + value.radius/2) and z1 + z2 >= (value.pos.z + value.radius/2)) or not includeZ:

				v = [
					# Top left
					[value.pos.x - value.length/2, value.pos.y + value.radius/2, value.pos.z - value.radius/2], 
					# Bottom left
					[value.pos.x - value.length/2, value.pos.y - value.radius/2, value.pos.z - value.radius/2], 
					# Bottom right
					[value.pos.x + value.length/2, value.pos.y - value.radius/2, value.pos.z - value.radius/2], 
		...
		elif obstruction_shape == 'sphere':
			if (z1 <= (value.pos.z + value.radius/2) and z1 + z2 >= (value.pos.z + value.radius/2)) or not includeZ:

				v = [
					# Top left
					[value.pos.x - value.radius/2, value.pos.y + value.radius/2, value.pos.z - value.radius/2], 
					# Bottom left
					[value.pos.x - value.radius/2, value.pos.y - value.radius/2, value.pos.z - value.radius/2], 
					# Bottom right
					[value.pos.x + value.radius/2, value.pos.y - value.radius/2, value.pos.z - value.radius/2], 
...
def nodeInObstruction(obstructions, x, y, z):

	if obstruction_shape == 'box':		
		for key, value in obstructions.items():
			if x >= value.pos.x - value.length/2 and x <= value.pos.x + value.length/2:
				if y >= value.pos.y - value.height/2 and y <= value.pos.y + value.height/2:
					if z >= value.pos.z - value.width/2 and z <= value.pos.z + value.width/2:
						return true
	elif obstruction_shape == 'cylinder':
		for key, value in obstructions.items():
			if x >= value.pos.x - value.length/2 and x <= value.pos.x + value.length/2:
				if y >= value.pos.y - value.radius/2 and y <= value.pos.y + value.radius/2:
					if z >= value.pos.z - value.radius/2 and z <= value.pos.z + value.radius/2:
						return true
	elif obstruction_shape == 'sphere':
		for key, value in obstructions.items():
			if x >= value.pos.x - value.radius/2 and x <= value.pos.x + value.radius/2:
				if y >= value.pos.y - value.radius/2 and y <= value.pos.y + value.radius/2:
					if z >= value.pos.z - value.radius/2 and z <= value.pos.z + value.radius/2:
						return true
...
#    g.get('A') to get a dict of links out of A, and g.get('A', 'B') to get the
#    length of the link from A to B.
...
if __name__ == "__main__":
	...
	for size in range(600, 2100, density):

		for obstructions in range(10, 60, 10):
		
			obs = RandomObstructions(num=range(obstructions), width=size, length=size, height=size, draw=draw)
			g = RandomGraphWithObstructions(obs, width=size, length=size, height=size, density=density, drawConnections=True, draw=draw)
			
			#print "Nodes: {0}".format(len(g.locations))
			#start = random.choice([k for k in g.locations])
			start = 0
			#goal = random.choice([k for k in g.locations])
			goal = random.choice(range(10, len(g.locations) - 1))

			#goal = random.choice(range(len(g.locations) - 20, len(g.locations) - 1))
			...
			# Order: number of nodes, size, number of obstructions, time, path length, nodes expanded
			print "%d,%d,%d,%f,%d,%d" % (len(g.locations), size, obstructions, (100*(t2-t1)), len(search.path()), len(problem.expanded))
			#print 100*(t2-t1)
			#print len(search.path())
			#print len(problem.expanded)
